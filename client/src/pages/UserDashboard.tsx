...
import { Component, Settings, Sparkles, Search } from "lucide-react";
...
const DashboardContent = () => {
  ...
  const handleSave = async () => {
    ...
    setError("");
    try {
      const response = await updateUser("self", {
      ...
      <div className="max-w-6xl mx-auto px-6 lg:px-8 space-y-16">
        ...
        <section className="grid grid-cols-1 md:grid-cols-12 gap-8 md:gap-16">
          ...
          <div className="md:col-span-8 grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div className="bg-bg-secondary border border-border-default rounded-2xl p-6 shadow-sm transition-colors relative overflow-hidden">
              ...
              <p className="text-xs uppercase tracking-widest text-text-muted font-bold mb-1 relative z-10">
                Enterprise Status
              </p>
              <p
                className={`text-base font-semibold capitalize relative z-10 ${user?.enterprise_status === "accepted" ? "text-accent-emerald" : "text-text-primary"}`}
              >
        ...
        <section className="grid grid-cols-1 md:grid-cols-12 gap-8 md:gap-16">
          <div className="md:col-span-4">
            ...
            <p className="text-sm text-text-secondary mt-2">
              Add specific hardware and software skills to get matched
              accurately with people who need your help.
            </p>
          </div>
          ...
          <div className="md:col-span-8 bg-bg-secondary border border-border-default rounded-3xl p-6 md:p-8 shadow-sm">
            <h3 className="text-sm font-bold text-text-primary mb-4 flex justify-between items-center">
              Active Skills
              <span className="text-xs font-medium bg-bg-tertiary px-2.5 py-1 rounded-full text-text-secondary">
                ...
                <div className="flex flex-wrap gap-2">
                  {selectedSkills.map((skill, index) => (
                    ...
                    >
                      ...
                      >
                        {skill}
                        <span className="ml-1.5 opacity-40 group-hover:opacity-100 transition-opacity">
                          ×
                        </span>
                      </Badge>
                    </button>
            ...
            <div className="border-t border-border-default pt-6">
              <h4 className="text-xs uppercase tracking-widest text-text-muted font-bold mb-4">
                Discover Skills
              </h4>
              <Input
        ...
        <section className="grid grid-cols-1 md:grid-cols-12 gap-8 md:gap-16">
          <div className="md:col-span-4">
            ...
            <p className="text-sm text-text-secondary mt-2">
              Tell the community about your past projects, specializations, and
              what drives you.
            </p>
          </div>
          <div className="md:col-span-8 bg-bg-secondary border border-border-default rounded-3xl p-6 md:p-8 shadow-sm">
        ...
        <div className="flex items-center justify-end gap-6 pt-8 pb-10">
          ...
          {saved && (
            <span className="text-sm font-semibold text-accent-emerald animate-pulse flex items-center gap-2">
              <svg
                className="w-4 h-4"
                fill="none"
                viewBox="0 0 24 24"
          ...
          <Button
            ...
            isLoading={isSaving}
            onClick={handleSave}
            className="min-w-48 shadow-lg shadow-accent-indigo/20"
          >
            Save Modifications
          </Button>
        </div>
